...
import com.trident.retail_analysis.query.CountryQuery;
...
public class Topology {
    ...
    private static final String SELECTED_COUNTRIES = "selected_countries";
    ...
    public static StormTopology buildTopology(TransactionalTridentKafkaSpout kafkaSpout, LocalDRPC localDRPC) {
        ...
        topology.newDRPCStream(RetailAnalysis.TOP_SOLD, localDRPC)
                .each(new Fields(DRPC_ARGUMENT),
                        new SplitBySeparator(RetailAnalysis.SEPARATOR),
                        new Fields(SELECTED_COUNTRIES))
                .stateQuery(totalVolume,
                        new Fields(SELECTED_COUNTRIES),
                        new CountryQuery(),
                        new Fields(CUSTOMER, COUNTRY, GROUPED_TRANSACTIONS))
                .each(new Fields(CUSTOMER, COUNTRY, GROUPED_TRANSACTIONS), new FilterNull())
